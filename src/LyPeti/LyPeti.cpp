  ...
  for(unsigned int r = 0; r < runs.size(); r++) {
    ...
    for(unsigned int ic = 0; ic < event.chambers()->size(); ic++) {
      ...
      for(unsigned int im = 0; im < mods.size(); im++) {
        ...
        //    int strips = event.numberOfStrips(chamber);
        //    double time = 0; double resolution = 0;
        //    std::vector<double> vTime; std::vector<double> vResolution;
        //    int goodStrips = 0; 
        //    //for(unsigned int c = 0; c < strips; c++) {
        //    for(unsigned int c = 25; c < 36; c++) {
        //    if(TH2s.find(nameTH2s.c_str())->second.ProjectionX("h", c, c)->GetEntries() > 150) {
        ...
        //      if(f1->GetParameter(2) < 0.5) { 
        //        time += f1->GetParameter(1);
        //        vTime.push_back(f1->GetParameter(1));
        //        resolution += f1->GetParameter(2);
        //        vResolution.push_back(f1->GetParameter(2));
        //        goodStrips += 1;
        //        // time
        //        nameTGEs = Form("%d%s%d%s%sgTimeProfile", chamber, strDelim.c_str(), c, strDelim.c_str(), mods.at(im).c_str());   
        //        itTGEs = dataRoot.addDqTGE(nameTGEs, &TGEs); itTGEs->second->config(nameTGEs, &_params);
        //        TGEs.find(nameTGEs.c_str())->second->setting(value, 0, f1->GetParameter(1),  f1->GetParError(1));
        //        // Resolution 
        //        nameTGEs = Form("%d%s%d%s%sgResolutionProfile", chamber, strDelim.c_str(), c, strDelim.c_str(), mods.at(im).c_str());   
        //        itTGEs = dataRoot.addDqTGE(nameTGEs, &TGEs); itTGEs->second->config(nameTGEs, &_params);
        //        TGEs.find(nameTGEs.c_str())->second->setting(value, 0, f1->GetParameter(2), f1->GetParError(2));
        //        
        //        nameTGEs = Form("%d%s%d%s%sgStripTimeProfile", chamber, strDelim.c_str(), (int)value, strDelim.c_str(), mods.at(im).c_str());
        //      }
        //    }
        //  }
        //  if(goodStrips > 0) {
        //    time = time/goodStrips;
        //    double sum = 0;
        //    for(unsigned int g = 0; g < goodStrips; g++)
        //      sum += (vTime.at(g) - time)*(vTime.at(g) - time);
        //    double eTime = std::sqrt(sum/goodStrips)/std::sqrt(goodStrips);
        //    
        //    resolution = resolution/goodStrips;
        //    sum = 0;
        //    for(unsigned int g = 0; g < goodStrips; g++)
        //      sum += (vResolution.at(g) - resolution)*(vResolution.at(g) - resolution);
        //    double eResolution = std::sqrt(sum/goodStrips)/std::sqrt(goodStrips);
        //    
        //    nameTGEs = Form("%d%s%sgTimeProfileAll", chamber, strDelim.c_str(), mods.at(im).c_str());   
        //    itTGEs = dataRoot.addDqTGE(nameTGEs, &TGEs); itTGEs->second->config(nameTGEs, &_params);
        //    TGEs.find(nameTGEs.c_str())->second->setting(value, 0, time, eTime);
        //    nameTGEs = Form("%d%s%sgResolutionProfileAll", chamber, strDelim.c_str(), mods.at(im).c_str());   
        //    itTGEs = dataRoot.addDqTGE(nameTGEs, &TGEs); itTGEs->second->config(nameTGEs, &_params);
        //    TGEs.find(nameTGEs.c_str())->second->setting(value, 0, resolution, eResolution);
        //  }
        //  //------   
        //}
      }
